...
function getTemplateId(folderId) {
  ...
  body
      ...
      .editAsText()
      .setBold(true);
  body.appendParagraph('- Action item: ').editAsText().setBold(true);
  ...
  for (i = 0; i < events.length; i++) {
    ...
    if (event.isOwnedByMe()) {
      ...
      if (conf) {
        ...
        for (var i in event.getGuestList(true)) {
          var guest = event.getGuestList(true)[i];
          // todo: look at getName and understand why it's empty
          var guestName = guest.getName();
          var guestEmail = guest.getEmail();
          var guestFull = guestName + ' - ' + guestEmail;
          
          body.insertParagraph(ref + 2, guestFull);
        }
